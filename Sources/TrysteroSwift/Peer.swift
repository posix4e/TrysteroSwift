...
class Peer: NSObject {
    ...
    private func handleSignalAsync(_ signal: Signal) async {
        ...
        case .offer:
            ...
            do {
                ...
                try await pc.setLocalDescription(pc.answer(for: nil))
                
                if let localSdp = pc.localDescription?.sdp {
                    onSignal(Signal(type: .answer, sdp: localSdp))
                }
            } catch {
    ...
    func sendData(type: String, data: Any) {
        ...
        if let stringData = data as? String {
            ...
            jsonData = (try? JSONSerialization.data(withJSONObject: data)) ?? Data()
        }
